...
def calculate_cost(df_result, is_antigen=False):
    ...
    # add changed pool size: HYD 0731
    ...
    if is_antigen:
        return {'RNA extraction consumables': 0., 'RT-PCR consumables': 0.,'Antigen test kits': antigen_per_test*total_tests.sum()+reporting_labor,
                ...
                'Number of changes in pool size': total_n_change, # HYD 0731
                'Average pool size change time (week)': (total_n_change+1)/with_test_days/7  # HYD 0731
                }
    else:
