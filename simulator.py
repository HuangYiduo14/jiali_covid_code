...
def sample_trajectories_from_file(n):
    '''
    sample trajectory parameters
    :param n: size of population
    :return:
    v_load_sampled: sampled virus load
    '''
    #global mcmc_result
    trajs = mcmc_result.sample(n,replace=True)
...
def SEIRsimulation(N, external_rate, D, L, infectiousness_function, asymptomatic=0.65, results_delay=0, R0=2.5,
                   cutoff=6, I_init=0, tmax=365, calibration_mode=False):
    '''
...
def SEIRsimulation_suppression(N,
    ...
    for t in range(tmax):
        ...
        Qt.append(len(Q))
        SQt.append(len(SQ))
    return St, It, Rt, Qt, SQt, np.sum(was_infected_ext), np.sum(
        was_infected_int), pre_control_external, pre_control_internal
